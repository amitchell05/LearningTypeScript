...
class InventoryStore {
  // Make these three properties private with the `private` keyword
  ...
  // Make _load() and _save() methods protected so they can be used by any class that derives from the IntentoryStore class
  protected _load() {
    return Promise.all([
...
// If you try to make a new instance and access its members, they will not show up on your list of auto-complete options
// If you try to use them, TypeScript will produce an error
// inventoryStore._save({} as any);

// TypeScript is a superset of JavaScript = it only extends JavaScript's functionality; it does not change how JavaScript works
// Despite TypeScript's complaining, line 193's code will get rendered in the trans-piled code and executeed at runtime

// Purpose of using TypeScript =>  listen to warning errors and correct them!
